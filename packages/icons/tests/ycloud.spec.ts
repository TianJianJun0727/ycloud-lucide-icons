...
describe('createIcons', () => {
  ...
  it('should only execute given provided context', () => {
    document.body.innerHTML = `<div id="context"><i data-ycloud="volume-2"></i></div><i data-ycloud="funnel"></i>`;
    ...
    const hasSvg = !!document.querySelector('svg.ycloud-volume-2');
    const hasUnreplaced = !!document.querySelector('i[data-ycloud="funnel"]');
  ...
  it('should add custom attributes', () => {
    document.body.innerHTML = `<i data-ycloud="volume-2" class="ycloud"></i>`;

    const attrs = {
      class: 'ycloud ycloud-volume-2 icon custom-class',
      fill: 'black',
    };
  ...
  it('should inherit elements attributes', () => {
    document.body.innerHTML = `<i data-ycloud="sun" data-theme-switcher="light"></i>`;
  ...
  it('should read elements from DOM and replace icon with alias name', () => {
    document.body.innerHTML = `<i data-ycloud="grid"></i>`;
  ...
  it('should not add aria-hidden attribute when a11y props are present', () => {
    document.body.innerHTML = `<i data-ycloud="volume-2" class="ycloud" aria-label="Volume"></i>`;
  ...
  it('should not replace icons inside template elements by default', () => {
    document.body.innerHTML = `<template><i data-ycloud="house"></i></template>`;
