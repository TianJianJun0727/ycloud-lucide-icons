---
title: 无障碍 - Angular
description: Angular 应用中图标无障碍使用的最佳实践。
---
...
只有当图标本身**承载了必要含义**时，才应该让它被辅助技术读取。下面会说明在 Angular 中如何做到这一点。

## 让图标可被辅助技术读取

如果需要把图标暴露给辅助技术，可以绑定图标组件的 `title` input，为图标提供可访问名称。

这样会移除 `aria-hidden` 属性，让屏幕阅读器可以读取这个图标。

```angular-html
<svg ycloudHouse title="这是我的房子" />
```
